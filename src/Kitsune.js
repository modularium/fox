...
class KitsuneParser {
    ...
    parseType(typeName, val, options) {
        ...
        let check = type.check(val, options)
    ...
    parse(args, usage) {
        ...
        */
        ...
        usage.forEach(({ type, required, count, ...otherOptions }) => {
            let argToParse = count ? args.slice(usageIndex, usageIndex + count) : args[usageIndex]
            let result

            if (!Array.isArray(type)) {
                if (!Array.isArray(argToParse)) 
                    result = this.parseType(type, argToParse, otherOptions)
                else {
                ...
                type.some(typeName => {
                    ...
                    if (!Array.isArray(argToParse)) 
                        resultType = this.parseType(typeName, argToParse, otherOptions)
                    else {
                ...
                if (!isParsed) { 
                    throw new FoxParserError(argToParse, usageIndex, type)
                }
            }
            ...
            if (Array.isArray(result)) {
                ...
                if (indices.length) {
                    throw new FoxParserError(argToParse[indices[0]], usageIndex + indices[0], type)
                }
